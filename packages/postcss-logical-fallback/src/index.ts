    ...
    return {
        ...
        async Once(root: Root, { AtRule }) {
            ...
            for (const check of [isInsetDecl, isBorderDecl, isFloatDecl]) {
                ...
                supports.walkDecls((decl) => {
                    if (!check(decl) || isCssVariable(decl.value)) {
                        decl.remove();
                    } else {
                        if (!supportsParam) {
        ...
        async Rule(rule: Rule) {
            ...
            if (rule.some((i) => i.type === 'decl' && isDeclWithRtlFallback(i))) {
                ...
                const insetInlineStart = cloned.nodes.find(
                    (i) => i.type === 'decl' && i.prop === 'inset-inline-start',
                ) as Declaration | undefined;
                const insetInlineEnd = cloned.nodes.find(
                    (i) => i.type === 'decl' && i.prop === 'inset-inline-end',
                ) as Declaration | undefined;
